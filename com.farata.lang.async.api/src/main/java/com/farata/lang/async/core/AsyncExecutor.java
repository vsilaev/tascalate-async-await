...
public class AsyncExecutor implements Serializable {
	...
	/**
	 * Execute the {@link Continuation} and start {@link Continuator} when the {@link Continuation} suspends.
	 */
	...
	protected @continuable <R, E extends Throwable> R awaitTask(final CompletionStage<R> future) throws E {
		...
		if (outcome instanceof Either) {
			...
			// Illegal wake-up
			throw new NoActiveAsyncCallException("Continuation was continued incorrectly - are your classes instrumented for javaflow?");
		}
	}
}
