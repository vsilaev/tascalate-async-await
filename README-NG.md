...
# Why async-await?
Asynchronous programming has long been an effective approach to execute tasks that don't need to block the flow or responsiveness of an application. Typically, these tasks fall into two categories: compute-bound operations or I/O-bound operations. Compute-bound operations involve performing calculations on a separate thread, allowing the main thread to carry on with its own execution. On the other hand, I/O-bound operations relate to external tasks, such as reading files or initiating network communication, that don't necessarily require blocking a thread while they progress.
...
1. Contextual variables need to be explicitly passed to callbacks. Although this issue can be mitigated to some extent with lambdas that capture lexical context, it may not entirely resolve the problem. In fact, when using numerous complex lambdas, the readability and clarity of your code may suffer.  
2. Managing asynchronous workflows through callbacks is challenging: branching logic within a chain of asynchronous callbacks becomes cumbersome, constructs like `try-with-resources` cannot be seamlessly applied, and common control flow mechanisms, including `try/catch` blocks for error-handling, need special treatment compared to synchronous code.  
3. Callbacks often execute on different threads. This imposes the need for careful handling of thread contexts during operation resumption - especially critical for applications running in managed environments like JEE or UI frameworks (JavaFX, Swing, etc).
