...
Armed with this chain, the library invokes the `resolve(...)` method on each provider in turn (from highest priority to the lowest) to obtain a `Scheduler`, stopping as soon as the first non-null outcome is encountered. Each `SchedulerProvider` makes use of the supplied parameters to determine the appropriate scheduler. These parameters include: 
